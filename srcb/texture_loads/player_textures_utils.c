...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/15 22:00:13 by zfarah            #+#    #+#             */
/*   Updated: 2025/07/15 22:00:40 by zfarah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_idle(t_ctx *ctx, t_tiles *tiles)
{
	...
	while (src[++i])
	{
		...
		if (!tiles->idle[i].txt)
			clean_exit(ctx, "Failed to load assets", errno);
		tiles->idle[i].img = mlx_texture_to_image(ctx->mlx, tiles->idle[i].txt);
		if (!tiles->idle[i].img)
			clean_exit(ctx, "Failed to load assets", errno);
...
void	load_left(t_ctx *ctx, t_tiles *tiles)
{
	...
	while (src[++i])
	{
		...
		if (!tiles->left[i].txt)
			clean_exit(ctx, "Failed to load assets", errno);
		tiles->left[i].img = mlx_texture_to_image(ctx->mlx, tiles->left[i].txt);
		if (!tiles->left[i].img)
			clean_exit(ctx, "Failed to load assets", errno);
...
void	load_up(t_ctx *ctx, t_tiles *tiles)
{
	...
	while (src[++i])
	{
		...
		if (!tiles->up[i].txt)
			clean_exit(ctx, "Failed to load assets", errno);
		tiles->up[i].img = mlx_texture_to_image(ctx->mlx, tiles->up[i].txt);
		if (!tiles->up[i].img)
			clean_exit(ctx, "Failed to load assets", errno);
...
void	load_down(t_ctx *ctx, t_tiles *tiles)
{
	...
	while (src[++i])
	{
		...
		if (!tiles->down[i].txt)
			clean_exit(ctx, "Failed to load assets", errno);
		tiles->down[i].img = mlx_texture_to_image(ctx->mlx, tiles->down[i].txt);
		if (!tiles->down[i].img)
			clean_exit(ctx, "Failed to load assets", errno);
