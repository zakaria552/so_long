...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/01 14:43:54 by zfarah            #+#    #+#             */
/*   Updated: 2025/07/01 17:05:39 by zfarah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_map_borders(t_ctx *ctx, t_asset *walls, int size)
{
	...
	while (++y < ctx->map->bounds[1] - 1)
	{
		...
		mlx_image_to_window(ctx->mlx, walls[0].img, (ctx->map->bounds[0] - 1)
			* size, y * size);
	}
	y = 0;
	x = 0;
	while (++x < ctx->map->bounds[0] - 1)
	{
		...
		mlx_image_to_window(ctx->mlx, walls[1].img, x * size,
			(ctx->map->bounds[1] - 1) * size);
	}
	mlx_image_to_window(ctx->mlx, walls[1].img, 0 * size, (ctx->map->bounds[1]
			- 1) * size);
	mlx_image_to_window(ctx->mlx, walls[1].img, (ctx->map->bounds[0] - 1)
		* size, (ctx->map->bounds[1] - 1) * size);
}
	...
	while (++y < map->bounds[1] - 1)
	{
		...
		while (++x < ctx->map->bounds[0] - 1)
		{
			if (grid[y][x] == '1' && grid[y + 1][x] != '1')
				mlx_image_to_window(ctx->mlx, tiles->walls[1].img, x
					* map->size, y * map->size);
			else if (grid[y][x] == '1' && grid[y + 1][x] == '1')
				mlx_image_to_window(ctx->mlx, tiles->walls[0].img, x
					* map->size, y * map->size);
			else
				mlx_image_to_window(ctx->mlx, tiles->floors[(x * y) % 4].img, x
					* map->size, y * map->size);
			if (grid[y][x] == 'C')
				mlx_image_to_window(ctx->mlx, tiles->orbs->img, x * map->size, y
					* map->size);
		}
	}
}
...
void	draw_player(t_ctx *ctx)
{
	...
	ctx->player->vision->img = mlx_new_image(ctx->mlx, ctx->map->width,
			ctx->map->height);
...
void	draw_exit(t_ctx *ctx)
{
	...
	mlx_image_to_window(ctx->mlx, tiles->doors[1].img, exit->x * ctx->map->size,
		exit->y * ctx->map->size);
	mlx_image_to_window(ctx->mlx, tiles->doors[0].img, exit->x * ctx->map->size,
		exit->y * ctx->map->size);
